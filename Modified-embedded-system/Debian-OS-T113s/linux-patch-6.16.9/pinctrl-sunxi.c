			  ...
			  u8 config)
{
	...
	pr_info("MUX pin=%u config=%u reg=0x%x shift=%u\n",
		pin + pctl->desc->pin_base,
		config,
		reg,
		shift);
	...
	val = (val & ~mask) | (config << shift);
	writel(val, pctl->membase + reg);

	pr_info("MUX REG AFTER = %08x\n",
		readl(pctl->membase + reg));
...
static int sunxi_pmx_gpio_set_direction(struct pinctrl_dev *pctldev,
...
{
	pr_info("SET_TYPE hwirq=%lu type=%x\n",
		irqd_to_hwirq(d),
		type);
	...
	regval = readl(pctl->membase + reg);
	pr_info("CFG AFTER = %08x\n", regval);

	{
		u32 status;

		status = readl(pctl->membase +
			       sunxi_irq_status_reg(pctl->desc, d->hwirq));

		pr_info("STATUS AFTER SET_TYPE = %08x\n", status);
	}
...
static void sunxi_pinctrl_irq_mask(struct irq_data *d)
{
	...
	val = readl(pctl->membase + reg);
	pr_info("CTRL BEFORE = %08x\n", val);

	writel(val & ~(1 << idx), pctl->membase + reg);

	val = readl(pctl->membase + reg);
	pr_info("CTRL AFTER  = %08x\n", val);
...
static void sunxi_pinctrl_irq_handler(struct irq_desc *desc)
{
	...
	/* Registro de máscara (CTRL) */
	reg = sunxi_irq_ctrl_reg_from_bank(pctl->desc, bank);
	ctrl = readl(pctl->membase + reg);
	pr_info("BANK=%lu CTRL=%08x\n", bank, ctrl);

	/* Registro de estado (STATUS) */
	reg = sunxi_irq_status_reg_from_bank(pctl->desc, bank);
	val = readl(pctl->membase + reg);
	pr_info("BANK=%lu STATUS=%08lx\n", bank, val);

	if (val) {
		int irqoffset;

		for_each_set_bit(irqoffset, &val, IRQ_PER_BANK) {
			pr_info("IRQ OFFSET=%d\n", irqoffset);
...
int sunxi_pinctrl_init_with_flags(struct platform_device *pdev,
				  ...
				  unsigned long flags)
{
	pr_info("### PINCTRL MODIFICADO CARGADO ###\n");
	...
	for (i = 0; i < pctl->desc->irq_banks; i++) {
		...
		pr_info("PARENT IRQ[%d] = %d\n", i, pctl->irq[i]);
		pr_info("PIO BANK %d parent irq = %d\n",
				i,
				pctl->irq[i]);
	...
	for (i = 0; i < pctl->desc->irq_banks; i++) {
		...
		pr_info("Register chained handler bank %d irq %d\n",
				i,
				pctl->irq[i]);
		irq_set_chained_handler_and_data(pctl->irq[i],
						 sunxi_pinctrl_irq_handler,
						 pctl);
		pr_info("CHAINED HANDLER INSTALLED irq=%d\n", pctl->irq[i]);
	}
